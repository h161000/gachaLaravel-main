            ...
            if($channel == 'sms') {
                $client->messages->create(
                    ...
                    [
                        ...
                        'body' => 'Raffle-proの認証コードは '. $code . ' です。',
                        // 'body' => ' '.$code.' です。',
                    ]);
                return true;
...
if ( ! function_exists('sendEmail')){
    ...
    {
        $content = 'RAFFLE-PROをご利用頂きありがとうございます。
...
if ( ! function_exists('getVideo')){
    function getVideo($gacha_point, $point)
    {
        $rate = $point * 100 / $gacha_point;
        $rate_limit = [0, 25, 50, 75, 100, 200, 300, 500, 1000, 3000];
        $video_index = 0;
        for ( ; $video_index < count($rate_limit); $video_index ++) {
            if ($rate <= $rate_limit[$video_index]) break;
        }
