    ...
    <AdminLayout>
            ...
            <div class="mb-6">
                <div v-for="(item, key) in products.data" class="py-1  border-neutral-200 border-b">
                    <button @click="showProductDetail(item)" class="flex border-neutral-200 items-center w-full">
                        ...
                        <div class="cursor-pointer flex items-center justify-between flex-1 text-xs py-1 px-2 ">
                            <div class="text-left">
                                <div>{{item.name}}</div>
                                <div>{{item.rare}}</div>
                                ...
                                <!-- <div>追跡番号: {{item.status_product}}</div> -->
                            </div>
        ...
        <div v-if="page_step==1" class="pt-6 md:px-2 px-4 mb-6" style="min-height:400px">  
            ...
            <div class="mb-6 w-full text-center">
                ...
                <button type="button" @click="submit(product.id)" :class="{ 'opacity-50': form.processing }" :disabled="form.processing" class="inline-block items-center px-8 py-2.5 bg-neutral-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-neutral-700 active:bg-neutral-700 focus:outline-none focus:border-neutral-700 focus:shadow-outline-neutral transition ease-in-out duration-150 m-1">
                    未発送に戻す
                </button>
